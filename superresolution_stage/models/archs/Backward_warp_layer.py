...
class Backward_warp(nn.Module):
    ...
    def _interpolate(self,img , x, y , out_height, out_width):
        ...
        x0 = x0_f.clone().cuda().long()
        y0 = y0_f.clone().cuda().long()
        x1 = torch.clamp(x1_f, 0, width_f -1).clone().cuda().long()
        y1 = torch.clamp(y1_f, 0, height_f -1).clone().cuda().long()
        ...
        dim1 = width * height
        dim2 = width
        base = (torch.arange(num_batch) * dim1).clone().cuda().long()
